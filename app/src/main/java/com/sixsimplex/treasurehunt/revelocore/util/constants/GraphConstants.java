package com.sixsimplex.treasurehunt.revelocore.util.constants;

public class GraphConstants {

    public static final String ENTITY = "entity";
    public static final String ENTITIES_TYPE = "type";
    public static final String _LABEL = "_label";

    public static final String _IN_V = "_inV";
    public static final String _OUT_V = "_outV";

    public static final String LABEL = "label";
    public static final String W9_ID_PROPERTY = "w9IdPro";
    public static final String W9_ID_LABEL = "w9IdLabel";
    public static final String CATEGORY_PROPERTY_NAME = "categoryName";
    public static final String ABBR = "abbr";
    public static final String IS_LOCKED = "isLocked";
    public static final String SELECTED_RENDERER_NAME = "renderName";

    public static final String DIRECTED = "directed";

    public static final String _ID = "_id";
    public static final String _TYPE = "_type";

    public static final String NAME = "name";
    public static final String TYPE = "type";
    public static final String V_LABEL = "vLabel";
    public static final String MODE = "mode";
    public static final String ENTITY_TYPE = "entityType";
    public static final String GEOMETRY_TYPE = "geometryType";
    public static final String ID_PROPERTY = "idProperty";
    public static final String IS_REFERENCE = "isReference";
    public static final String PROPERTIES_LIST = "propertiesList";
    public static final String DESCRIPTION = "description";

    public static final String FROM_ID = "fromId";
    public static final String TO_ID = "toId";
    public static final String FROM = "from";
    public static final String TO = "to";

    public static final String IN_V = "_inV";
    public static final String OUT_V = "_outV";

    public static final String VERTICES = "vertices";
    public static final String EDGES = "edges";
    public static final String VERTEX = "vertex";
    public static final String GRAPH = "graph";
}