...
import com.sixsimplex.treasurehunt.revelocore.upload.UploadLogs;
...
public class UploadEditedFeature {
    ...
    public UploadEditedFeature(Context context, UploadInterface uploadInterface, String entityName, CMGraph cmGraph, String w9Id, String featureLabel, boolean hasAttachments, boolean uploadTraversalGraph) {
            ...
            if(payloadJson.has("status")){
                if(payloadJson.getString("status").equalsIgnoreCase("success")){
                    if (!payloadJson.has("dataJson")) {
                        ...
                        uploadFeatureToServerData(AppConstants.EDIT, dataJson);
                    }
                }else{
                    updateMetaDataEntryIfFailed();
                    uploadInterface.OnUploadFinished(false, payloadJson);
        ...
        try {
            ...
            try {
                ...
                if (cmGraph != null && addedIdMap != null && !addedIdMap.isEmpty()) {
                    JSONObject payload = createFeatureJson(surveyName, addedIdMap, cmGraph, AppConstants.EDIT);//create edit json
                    if (payload != null) {
    ...
    private void updateMetaDataEntryIfFailed(){
        try {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    EditMetaDataTable.insertEditEntry(entityName, w9Id, featureLabel, context);
                }
            }).start();
        }catch (Exception e){
            e.printStackTrace();
